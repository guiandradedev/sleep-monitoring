...
def websocket(ws):
    ...
    while True:
        ...
        try:
            #print("bytes recebidos:", raw_data.hex())
            
            # Garantir que são exatamente 20 bytes (1 pacote)
            if len(raw_data) != 1004:
                print(f"⚠️ Pacote inesperado (tamanho {len(raw_data)} bytes)")
                continue

            # Desempacotar: 1 uint32 + 8 int16 => 'I8h'
            timestamp, *samples = struct.unpack('<I500h', raw_data)
            ...
            try:
                ...
                us_timestamp = timestamp * 1000  # converter para milissegundos
                # Gera uma lista de tuplas (timestamp, sample, type)
                
                rows = [(int(us_timestamp - 500 * 125 + i * 125), int(sample), "microphone")
                ...
                """ for i, sample in enumerate(samples):
                    # timestamp + i * 0.125ms (amostras a cada 125μs)
                    sample_timestamp = us_timestamp -500 * 125 + i * 125
                    cursor.execute(
                        "INSERT INTO data (sample, timestamp, type) VALUES (%s, %s, %s)",
                        (sample, sample_timestamp, "microphone")
                    ) """
